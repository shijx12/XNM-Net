...
def main(args):
    print('Loading data')
    questions = json.load(open(args.input_questions_json, 'r'))
    ...
    if args.mode == 'train':
        ...
        answer_cnt = {}
        for q in questions:
            answers = [_['answer'] for _ in q['answers']]
            for i,answer in enumerate(answers):
                answer = process_punctuation(answer)
                answer_cnt[answer] = answer_cnt.get(answer, 0) + 1
                answers[i] = answer
            q['answers'] = answers # update
        answer_token_to_idx = {}
