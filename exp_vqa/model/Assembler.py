...
class Assembler:
    def __init__(self, module_vocab):
        self.module_names = [module_vocab['program_idx_to_token'][i] for i in range(len(module_vocab['program_idx_to_token']))]
        # find the index of <eos>
        self.EOS_idx = module_vocab['program_token_to_idx']['<eos>']
        self.name2idx_dict = module_vocab['program_token_to_idx']
