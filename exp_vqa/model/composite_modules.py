...
class QueryOrCountModule(nn.Module):
    """
    ...
    def forward(self, attn, cat_matrix, edge_cat_vectors, query, feat):
        attribute_attn = self.relate(attn, cat_matrix, edge_cat_vectors, query, None)
        out_1 = torch.matmul(attribute_attn, feat)
...
class SimpleFind2Module(nn.Module):
    def __init__(self, dim_v):
        super().__init__()
        self.attendNode = AttendNodeModule()
        self.transConn = TransConnectModule()

    def forward(self, conn_matrix, feat, query):
        attribute_attn = self.attendNode(feat, query)
        out = self.transConn(attribute_attn, conn_matrix)
        return out
        
class SimpleFindModule(nn.Module):
    def __init__(self, dim_v):
        super().__init__()
        self.attendNode = AttendNodeModule()

    def forward(self, conn_matrix, feat, query):
        attn = self.attendNode(feat, query)
        return attn

class SimpleDescribeModule(nn.Module):
    def __init__(self, dim_v):
        super().__init__()
        self.count = ExistOrCountModule(dim_v)
        self.gate = nn.Sequential(
    ...
    def forward(self, attn, query, feat):
...
class SimpleRelateModule(nn.Module):
    def __init__(self, dim_v):
        super().__init__()
        self.attendEdge = AttendEdgeModule()
        self.transWeit = TransWeightModule()
    ...
    def forward(self, attn, cat_matrix, edge_cat_vectors, query):
        weit_matrix = self.attendEdge(edge_cat_vectors, query, cat_matrix)
        out = self.transWeit(attn, weit_matrix)
        weight = self.gate(query)
        out = torch.matmul(weight, torch.stack([out, attn]))
